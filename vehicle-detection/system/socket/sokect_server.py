import threading
import socketio
from system.utils import get_ipv4_address, get_camera_data
import eventlet
...
class SocketIOServer:
    ...
    def handle_message(self, sid, data):
        ...
        if data['status'] == 'start-record':
            self.logic_handlers[data['camera_id']]._is_clicked_alarm = True
        
        elif data['status'] == 'update-polygon':
            print("logic polygon")
            camera_ids, polygons = get_camera_data()
            self.logic_handlers[data['camera_id']]._points = polygons[data['camera_id']]
            print(polygons[data['camera_id']])
