...
def scaling_polygon(cameras):
    try:
        points_dict = {}
        for camera in cameras:
            points = {}
            
            for idx, polygon in enumerate(camera['polygons'], start=2):
                canvas_width = polygon['image_width']
                canvas_height = polygon['image_height']
                
                # Calculate scaling factors
                x_scale = canvas_width / camera['output_width']
                y_scale = canvas_height / camera['output_height']
                
                # Scale the coordinates
                scaled_x_y_pairs = [
                    [int(x * x_scale * canvas_width), int(y * y_scale * canvas_height)]
                    for x, y in zip(polygon['center_polygon'][::2], polygon['center_polygon'][1::2])
                ]
                
                points[f'POINTS_{idx}'] = scaled_x_y_pairs
            
            points_dict[camera['camera_id']] = points
        return points_dict
    except: 
...
def get_camera_data():
    ...
    content = res.json()
    return [camera['camera_id'] for camera in content['data']['cameras']], scaling_polygon(content['data']['cameras'])
